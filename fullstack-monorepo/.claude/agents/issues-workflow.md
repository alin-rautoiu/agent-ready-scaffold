...
description: (Claude) Use to build a ranked backlog from open issues, or to execute a ranked subset of issues by delegating to the Orchestrator in sequence.
...
If you need a tool that is not available (e.g. an issue tracker CLI not installed or not authenticated), **stop and tell the user** what tool you need, why, and what they should do to provide access. Do not invent ad-hoc alternatives.
