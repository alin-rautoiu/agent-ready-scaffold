...
description: (Codex) Use the shared issue workflow runner to generate a ranked backlog or execute a ranked issue subset with an explicit target.
---
...
<!-- TODO: if the project has a canonical workflow script, specify it here. e.g.:
  "Use `<run-command> issues:workflow -- --repo=<owner/name> [--mode=<backlog|execute>] [flags]` as the canonical runner."
...
<!-- TODO: replace with this project's actual backlog and execute commands, e.g.:
  "1. Backlog mode: `<run-command> issues:workflow -- --repo=<owner/name> [--issues=1,2]`"
  "2. Execute mode: `<run-command> issues:workflow -- --mode=execute --repo=<owner/name> --target=<target> [flags]`"
